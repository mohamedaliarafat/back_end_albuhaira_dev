...
exports.verifyOtpAndLogin = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const profile = await CompleteProfile.create({ user: user._id });
      user.cart = cart._id;
      ...
      if (!profile) {
        profile = await CompleteProfile.create({ user: user._id });
        user.completeProfile = profile._id;
        await user.save();
      }
    }

    const token = jwt.sign({ id: user._id, phone: user.phone, userType: user.userType }, process.env.JWT_SECRET, { expiresIn: "7d" });
    ...
    res.json({ success: true, message: "تم تسجيل الدخول بنجاح ✅", data: user, token });
  } catch (err) {
...
/* ======================================================
   🏠 إدارة العناوين
   ✅ الآن متوافق مع verifyPhone middleware
====================================================== */
exports.addAddress = async (req, res) => {
  try {
    const userId = req.user.id; // جاء من verifyPhone
    const { addressLine1, city, district, state, country, postalCode, isDefault, deliveryInstructions, latitude, longitude } = req.body;
    ...
    const isProfileCompleted =
      profile && profile.email && profile.commercialLicense && profile.energyLicense &&
      profile.commercialRecord && profile.taxNumber && profile.nationalAddress && profile.civilDefenseLicense;

    if (!isProfileCompleted)
      return res.status(403).json({ success: false, message: "يرجى إكمال الملف الشخصي قبل إضافة عنوان جديد" });

    if (isDefault) await Address.updateMany({ userId }, { isDefault: false });

    const newAddress = await Address.create({ userId, addressLine1, city, district, state, country, postalCode, isDefault, deliveryInstructions, latitude, longitude });
...
exports.getUserNotifications = async (req, res) => {
  try {
    ...
    const userNotifications = notifications.map(n => ({ ...n, isRead: n.readBy?.includes(userId) || false }));
    res.json({ success: true, message: "تم جلب الإشعارات بنجاح ✅", notifications: userNotifications });
  } catch (err) {
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "تم جلب جميع المستخدمين ✅", users });
  } catch (err) {
...
exports.toggleUserStatus = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: user.isActive ? "تم تفعيل المستخدم" : "تم إيقاف المستخدم" });
  } catch (err) {
